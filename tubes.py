...
def hitungTotal():
    ...
    for i in range(1, len(riwayat)):  
        ...
        if not riwayat[i]:
            ...
            print(f"{'🍽️  Nama Barang':<25}{'📦jumlah':<10} {'💵 Harga'}")
            for j in range(len(riwayat[i])):
                print(f"{riwayat[i][j]:<25} {allpay[i][j][0]:<10} Rp. {(allpay[i][j][0] * allpay[i][j][1]):,}")
                total += (allpay[i][j][0] * allpay[i][j][1])
            print()
...
def history(save_i):
    ...
    if not allpay[save_i]:
        ...
        for i in range(len(riwayat[save_i])):
            print(f"{i+1}. {riwayat[save_i][i]:<30} {allpay[save_i][i][0]:<2} Rp. {(allpay[save_i][i][1] * allpay[save_i][i][0]):,}")
        print("-"*100)
...
def payment(i_user):
    ...
    for j in range(len(order[i_user])):
        nama = order[i_user][j]
        jumlah = pay[i_user][j][0]
        harga_satuan = pay[i_user][j][1]
        harga_total = jumlah * harga_satuan
        print(f"{j+1}. {nama:<30} {jumlah:<2} x Rp.{harga_satuan:<8,} =  Rp.{harga_total:,}")
        total_pay += harga_total
    print("-"*65)
    ...
    while flag:
        ...
        for i in range(len(menu)):
            if pesan in menu[i]:
                ...
                if menu[i][1] in order[i_user]:
                    idx = order[i_user].index(menu[i][1])
                    pay[i_user][idx][0] += jumlah  
                else:
                    order[i_user].append(menu[i][1])
                    pay[i_user].append([jumlah, harga])

                print(f"✅ {menu[i][1]} sebanyak {jumlah} berhasil ditambahkan ke pesanan kamu.")
                flag = False
                break
        if not check:
